...
export const InputToken = ({ token }) => {
  ...
  useEffect(() => {
    if (debouncedTokenAmount !== null) {
      dispatch(setIsLoadingQuote(false));
      const parsedAmount =
        Number(debouncedTokenAmount) * 10 ** token.tokenDecimals;
      dispatch(setInputAmount(parsedAmount));
    }
  }, [debouncedTokenAmount, dispatch, token.tokenDecimals]);

  const handleClickAmount = (amount) => {
    setTokenAmount(amount);
    dispatch(setInputAmount(amount * 10 ** token.tokenDecimals));
  };
      ...
      <div className="flex flex-col items-end w-full">
        <div className="flex flex-row gap-2">
          ...
          <button
            onClick={() =>
              handleClickAmount(
                (token.tokenBalance / 10 ** token.tokenDecimals / 2).toFixed(2)
              )
            }
            className="px-2 rounded-xl text-xs bg-gray-700 text-gray-200"
          >
